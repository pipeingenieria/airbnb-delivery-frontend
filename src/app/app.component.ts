...
export class AppComponent {
 ...
 toastMessage = signal<string | null>(null);
  ...
  // BASE DE DATOS: Catálogos con valores convertidos a USD e imágenes corregidas
  ...
  setCategory(category: 'all' | 'restaurants' | 'liquor' | 'pharmacy' | 'experiences') {
...
showToast(message: string) {
    ...
    // Si ya había un mensaje, limpiamos el temporizador para que no se oculte antes de tiempo
  ...
  removeFromCart(productId: number) {
    ...
    if (existingProduct) {
      ...
      // Mostramos qué producto se redujo
  ...
  removeItemFully(productId: number) {
    ...
    if (itemToRemove) {
      this.showToast(`${itemToRemove.name} removed`);
    }
